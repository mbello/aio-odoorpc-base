...
async def db_exist(http_client: T_AsyncHttpClient, url: str, *_,
    ...
    db_name = _[0] if db_name is None and len(_) == 3 and isinstance(_[1], int) else db_name
    
    if db_name is None:
        raise RuntimeError("[db_exist] No database name provided.")
    
    return await rpc_result(http_client, url, service=__SERVICE, method='db_exist',
                            args=[db_name], ensure_instance_of=bool)


async def list_countries(http_client: T_AsyncHttpClient, url: str, master_password: str) -> List[str]:
    return await rpc_result(http_client, url, service=__SERVICE, method='list_countries',
                            args=[master_password], ensure_instance_of=list)


async def list_databases(http_client: T_AsyncHttpClient, url: str, *_,
                         document: Optional[bool] = None) -> List[str]:
    return await rpc_result(http_client, url, service=__SERVICE, method='list',
                            args=None if document is None else [document],
                            ensure_instance_of=list)


async def list_lang(http_client: T_AsyncHttpClient, url: str, *_) -> List[str]:
    return await rpc_result(http_client, url, service=__SERVICE, method='list_lang',
                            ensure_instance_of=list)


async def server_version(http_client: T_AsyncHttpClient, url: str, *_) -> str:
    return await rpc_result(http_client, url, service=__SERVICE, method='server_version',
...
async def change_admin_password(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
                                new_password: str) -> bool:
    # web/database/change_password / POST form-encoded / params master_pwd, master_pwd_new
    return await rpc_result(http_client, url, service=__SERVICE, method="change_admin_password",
                            args=[master_password, new_password],
                            ensure_instance_of=bool)


async def create_database(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
    ...
    return await rpc_result(http_client, url, service=__SERVICE, method="create_database",
                            args=args, ensure_instance_of=bool)


async def drop_database(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
                        db_name: str) -> bool:
    return await rpc_result(http_client, url, service=__SERVICE, method="drop",
                            args=[master_password, db_name],
                            ensure_instance_of=bool)


async def dump_database(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
                        db_name: str, format: str = "zip") -> bytes:
    res = await rpc_result(http_client, url, service=__SERVICE, method='dump',
                           args=[master_password, db_name, format], ensure_instance_of=str)
    return base64.b64decode(res)


async def duplicate_database(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
                             db_original_name: str, db_name: str) -> bool:
    return await rpc_result(http_client, url, service=__SERVICE, method='duplicate_database',
                            args=[master_password, db_original_name, db_name], ensure_instance_of=bool)


async def migrate_databases(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
                            databases: List[str]) -> bool:
    return await rpc_result(http_client, url, service=__SERVICE, method='migrate_databases',
                            args=[master_password, databases], ensure_instance_of=bool)


async def rename_database(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
                          old_name: str, new_name: str) -> bool:
    return await rpc_result(http_client, url, service=__SERVICE, method='rename',
                            args=[master_password, old_name, new_name], ensure_instance_of=bool)


async def restore_database(http_client: T_AsyncHttpClient, url: str, master_password: str, *,
    ...
    return await rpc_result(http_client, url, service=__SERVICE, method='restore',
