...
async def db_exist(http_client: T_AsyncHttpClient, url: str = '', *, db_name: str) -> bool:
    ...
    return await rpc_result(http_client, url, service=service, method=method, args=args, ensure_instance_of=bool)
...
async def list_lang(http_client: T_AsyncHttpClient, url: str = '') -> List[str]:
    ...
    return await rpc_result(http_client, url, service=service, method=method, ensure_instance_of=list)
...
async def create_database(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
    ...
    args = [admin_password, db_name, demo, lang, user_password, login, country_code, phone]

    return await rpc_result(http_client, url, service=service, method=method, args=args, ensure_instance_of=bool)


async def duplicate_database(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
...
async def drop_database(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
                        db_name: str) -> bool:
...
async def dump_database(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
                        db_name: str, format: str = "zip") -> bytes:
    service, method = __SERVICE, 'dump'
    args = [admin_password, db_name, format]
    
    res = await rpc_result(http_client, url, service=service, method=method, args=args, ensure_instance_of=str)
    return res.encode()


async def restore_database(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
...
async def rename_database(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
...
async def migrate_databases(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
...
async def change_admin_password(http_client: T_AsyncHttpClient, url: str = '', *, admin_password: str,
